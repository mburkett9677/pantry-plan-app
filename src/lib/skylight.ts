...
export async function syncMealsToSkylight(opts: {
  ...
  // Replace prior entries for this week so re-sync doesn't stack duplicates.
  const dates = opts.meals.map((m) => m.date).sort();
  if (dates.length) {
    ...
    const targets = new Set(
      opts.meals.map((m) => `${m.date}|${m.slot}`),
    );
    ...
    const categoryBySlot = new Map(
      [...categoryCache.entries()].filter(([, id]) => Boolean(id)) as Array<
        [string, string]
      >,
    );
    const slotByCategory = new Map(
      [...categoryBySlot.entries()].map(([slot, id]) => [id, slot]),
    );

    for (const sitting of existing) {
      const slot = sitting.mealCategoryId
        ? slotByCategory.get(sitting.mealCategoryId)
        : undefined;
      if (!slot) continue;

      const datesToClear = sitting.date
        ? targets.has(`${sitting.date}|${slot}`)
          ? [sitting.date]
          : []
        : opts.meals.filter((m) => m.slot === slot).map((m) => m.date);
      ...
      for (const date of datesToClear) {
        ...
        if (!del.ok && del.status !== 404) {
          ...
          warnings.push(
            `Could not clear old Skylight meal ${date}: ${del.status} ${text.slice(0, 80)}`,
          );
        }
      }
    }
