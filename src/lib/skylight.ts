...
export async function syncMealsToSkylight(opts: {
  ...
  for (const meal of opts.meals) {
    try {
      ...
      // Skylight has no separate instructions field — directions go in
      // `description` on the recipe, and we also copy them onto the sitting so
      // they show when you open the meal on the calendar.
      ...
      if (!instructions && !ingredientLines.length) {
        warnings.push(
          `${meal.title} has no instructions/ingredients in PantryPlan (link a recipe, then sync again)`,
        );
      }
        ...
        `/api/frames/${opts.frameId}/meals/sittings?include=meal_category,meal_recipe`,
        {
          ...
          body: JSON.stringify(
            compactBody({
              ...
              // Visible on the calendar meal card / detail in Skylight.
              description: description || undefined,
              note: instructions || (meal.title !== recipeTitle ? meal.title : undefined),
            }),
          ),
        },
      );
